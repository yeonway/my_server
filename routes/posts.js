...
router.post("/:id/comment", authMiddleware, contentFilter, async (req, res) => {
  try {
    ...
    try {
      ...
      const commentId = savedComment && savedComment._id ? savedComment._id.toString() : null;
      const linkTarget = commentId ? `/posts.html?post=${postId}#comment-${commentId}` : `/posts.html?post=${postId}`;
      ...
      if (post.author && post.author.toString() !== req.user.id) {
        await NotificationService.createNotification({
          ...
          actorId: req.user.id,
          type: 'comment',
          message: `${req.user.username}?? "${post.title || '???'}"? ??? ?????`,
          link: linkTarget,
          payload: { postId, commentId },
        });
      }

      const exclusion = [req.user.id];
      if (post.author) exclusion.push(post.author.toString());

      const mentionedUsers = await NotificationService.findMentionedUsers(req.body.content, {
        excludeIds: exclusion,
      });

      if (mentionedUsers.length && commentId) {
        await NotificationService.createNotifications(
          mentionedUsers.map((user) => ({
            ...
            link: linkTarget,
            payload: { postId, commentId, mentioned: user.username },
          }))
        );
      }
    } catch (notificationError) {
      console.error('[notifications] comment create', notificationError);
    }
