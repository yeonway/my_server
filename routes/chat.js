...
router.post('/rooms', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const roomName = (mapped && mapped.name) || name || 'Group chat';

      const notificationRecipients = participants

        .map((participant) => participant && participant.toString())

        .filter((participantId) => participantId && participantId !== req.user.id);
      ...
      if (notificationRecipients.length) {
        ...
        await NotificationService.createNotifications(
          ...
          notificationRecipients.map((recipientId) => ({
            ...
            message: `${req.user.username}?? "${roomName}" ????? ??????`,

            link: `/chat.html?room=${roomIdString}`,

            payload: { chatroomId: roomIdString, name: roomName },
      ...
      console.error('[notifications] group invite', notificationError);
